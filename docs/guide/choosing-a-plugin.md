...
| Search the web through a local instance | [SearXNG Search](/plugins/searxng-search) | Installs SearXNG in Docker, returns JSON results, and can store a local search receipt |
...
- SearXNG Search runs a local container; SearXNG sends queries to its configured web search engines.
