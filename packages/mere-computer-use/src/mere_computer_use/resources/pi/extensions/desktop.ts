...
export default function desktop(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    description: "Get a fresh screenshot and accessibility snapshot for the selected window. Observe once, then act; observe again after each action and after the final action.",
  ...
  pi.registerTool({
    ...
    description: "Click an accessibility token, or x/y screenshot pixels with the latest snapshot ID, in the selected window.",
    parameters: Type.Object({
    ...
    execute: async (_id, params) => result(await call("click", [
      ...
      ...(params.snapshotId ? ["--snapshot-id", params.snapshotId] : []),
    ])),
  });
  pi.registerTool({
    name: "desktop_type",
    label: "Type in selected-window element",
    description: "Type into an accessibility token, or x/y screenshot pixels with the latest snapshot ID, in the selected window.",
    parameters: Type.Object({
      elementToken: Type.Optional(Type.String()),
      x: Type.Optional(Type.Number()),
      y: Type.Optional(Type.Number()),
  ...
  pi.registerTool({
    ...
    description: "Press one key in the selected window, optionally with modifiers such as cmd for Command-S, and an element token from the latest observation.",
    parameters: Type.Object({ key: Type.String(), modifiers: Type.Optional(Type.Array(Type.String())), elementToken: Type.Optional(Type.String()) }),
    execute: async (_id, params) => result(await call("key", [
      "--key", params.key,
      ...(params.elementToken ? ["--element-token", params.elementToken] : []),
      ...(params.modifiers ?? []).flatMap((modifier) => ["--modifier", modifier]),
    ])),
  });
}
